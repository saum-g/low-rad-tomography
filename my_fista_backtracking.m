function X = my_fista_backtracking(calc_f, grad, Xinit, opts, calc_F)   
    ...
    while  iter < opts.max_iter
        ...
        while true 
            ...
            if F <= Q 
                ...
                temp=reshape(x_new,[opts.ht opts.width]);
                temp=idct2(temp);
                temp(temp<0)=0;
                x_new=dct2(temp);
                x_new=reshape(x_new,[opts.ht*opts.width 1]);
                cost_new = feval(calc_F, x_new);
                if cost_new <= cost_old
                    fprintf("\n")
                    break;
                else
                    fprintf("cost not decrease on modification")
                    e = norm1(x_new - x_old)/numel(x_new);
                    if e < opts.tol
                        fprintf(" new x very close\n");
                        break;
                    else
                        fprintf(" new x still far\n");
                    end
                end
            end
            Lbar = Lbar*opts.eta; 
